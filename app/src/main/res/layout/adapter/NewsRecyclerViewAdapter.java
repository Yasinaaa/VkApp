...
public class NewsRecyclerViewAdapter extends
        RecyclerView.Adapter<com.example.almaz.flatstackvk.adapter.NewsRecyclerViewAdapter.ViewHolder>{

    private List<com.example.almaz.flatstackvk.model.PostsResponse.Response.Item> mRecords;
    private HashMap<Long, com.example.almaz.flatstackvk.model.PostsResponse.Response.Group> mGroups = new HashMap<>();
    private HashMap<Long, com.example.almaz.flatstackvk.model.PostsResponse.Response.Profile> mProfiles = new HashMap<>();
    Context mContext;

    public NewsRecyclerViewAdapter(Context context,
                                   List<com.example.almaz.flatstackvk.model.PostsResponse.Response.Item> records,
                                   HashMap<Long, com.example.almaz.flatstackvk.model.PostsResponse.Response.Group> groups,
                                   HashMap<Long, com.example.almaz.flatstackvk.model.PostsResponse.Response.Profile> profiles){
    ...
    @Override
    public com.example.almaz.flatstackvk.adapter.NewsRecyclerViewAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(com.example.almaz.flatstackvk.adapter.NewsRecyclerViewAdapter.ViewHolder holder, final int position) {
        final com.example.almaz.flatstackvk.model.PostsResponse.Response.Item post = mRecords.get(position);
    ...
    public void setAuthorInfo(@NonNull long source_id,
        ...
        if (source_id < 0) {
            //if author is group
            com.example.almaz.flatstackvk.model.PostsResponse.Response.Group group = mGroups.get(source_id * (-1));
            authorNameView.setText(group.name);
            drawAuthorImage(group.photo_50, authorPhotoView);
        } else {
            //if author is user
            com.example.almaz.flatstackvk.model.PostsResponse.Response.Profile profile = mProfiles.get(source_id);
    ...
    @Nullable
    private String setPostImageAndGetUrl(@NonNull com.example.almaz.flatstackvk.model.PostsResponse.Response.Item post,
